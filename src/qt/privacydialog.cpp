                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zDRK ought to be enough for anybody." - Bill Gates, 2017
    ui->zDRKpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzDRKSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzDRK->setEnabled(false);
        ui->pushButtonMintzDRK->setToolTip(tr("zDRK is currently disabled due to maintenance."));

        ui->pushButtonSpendzDRK->setEnabled(false);
        ui->pushButtonSpendzDRK->setToolTip(tr("zDRK is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zDRKpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzDRK_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zDRK is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zDRK...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zDRK in ") + 
...
void PrivacyDialog::on_pushButtonSpendzDRK_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zDRK is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zDRK
        sendzDRK();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zDRK
    sendzDRK();
}

void PrivacyDialog::on_pushButtonZPivControl_clicked()
{
    ZPivControlDialog* zDrkControl = new ZPivControlDialog(this);
    zDrkControl->setModel(walletModel);
    zDrkControl->exec();
}

void PrivacyDialog::setZPivControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzDrkSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzDRK()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Darkcoin Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zDRKpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zDRK is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " DRK </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zDRK</b>";
    ...
    // use mints from zDrk selector if applicable
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zDrk Spend #: ") + QString::number(nCount) + ", ";
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zDRK </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zDRK "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zDRK "));
    ui->labelzDRKAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzDRKSyncStatus->setVisible(fShow);
}
